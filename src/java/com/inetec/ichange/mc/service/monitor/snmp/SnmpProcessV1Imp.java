...
public class SnmpProcessV1Imp implements ISnmpProcess {
    private static final Logger log = Logger.getLogger(SnmpProcessV1Imp.class);
    ...
    public void run() {
        if(isError){
            log.error("Device name is:"+devicename+" not found snmpoid data row.");
        }
        isRun = true;
        while (isRun) {
            try {
    ...
    public void monitorDevice() throws Exception {
        ...
        if (event != null && event.getResponse() != null) {
            ...
            log.info("Snmp Device response is null.");
        }
    }
